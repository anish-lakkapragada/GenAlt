...
const madeBy = 'Caption generated by GenAlt.';
...
port.onMessage.addListener((msg) => {
  let { url, caption } = msg; // caption is a string, not object
  const images = SRC_IMAGES[url];
  console.log(`for this src: ${images[0].src}, you get this caption: ${caption}`);
  for (const image of images) {
    if (caption === null) {
      ...
      console.warn(`this url: ${image.src} gives an error`);
      ERROR_SRCS[image.src] = true;
      image.alt = ORIGINAL_ALTS[image.src] || '';
...
function fix(params) {
  for (const image of document.querySelectorAll('img')) {
    if (NUM_TRIES >= MAX_CAPTIONS_DELIVERED) {
      console.log('MAX DONE STAHPPPPPP');
      image.alt = ORIGINAL_ALTS[image.src]; 
      revertAlt(); 
      continue; 
    }
    
    if (image.src.includes('data:image') || image.src.includes('svg') || image.src.includes('gif')) {
      continue;
    }
    ...
    if (image.alt == DEFAULT_ALT && IMAGE_ALTS[image.src] == undefined && ERROR_SRCS[image.src] == undefined) {
      ...
      if (image.width >= 75 && image.height >= 75) {
        NUM_TRIES++;
        console.log('trying this src: ' + image.src);
...
setInterval(() => {
  console.log(`num tries: ${NUM_TRIES} and max tries: ${MAX_CAPTIONS_DELIVERED}`);
}, 2000);
setInterval(() => {
  updateData(false);
